...
## Purpose
Persist per-session filtered artifacts and an evidence index under
`storage_root` with deterministic, expandable-by-ID evidence, so handoff
claims and MCP tools can reference original content without re-reading
whole transcripts, safely and concurrently across multiple processes.
