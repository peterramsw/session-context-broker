# session-context-mcp-server Specification

## Purpose
Expose the session-context broker's session/filter/handoff/evidence/workspace
operations to MCP clients (Claude Code, Codex, Google Antigravity) as a single
stdio server, sharing core logic with the CLI rather than duplicating it.
