...
### 良好使用方式：長時間中斷後（例如撞到 5 小時用量限額）

**在同一個對話串裡跑 `cc-session handoff` 沒有幫助。** 它是讀 transcript 檔案的外部工具，不會回寫進 agent 正在用的那個對話上下文；prompt cache 只有 5 分鐘 TTL，中斷超過這個時間，**回去接續原本那條對話串**時，第一次呼叫一定會把累積的完整 context 整包重新 cache write——無論你有沒有先跑過 `cc-session`，這筆成本都省不掉。

真正省的做法，是把「長時間中斷」當成邊界，**開一個新對話**：

1. 中斷前（或限額卡住當下），跑 `cc-session handoff <session-id> --llm auto`，把目前這個 session 過濾／蒸餾好
2. 限額解除後，**不要接續原本那條對話串**，改開一個新對話
3. 在新對話裡用 `get_handoff` / `filter_session`（或直接請 agent 「resume 上次那個 session」，交給 Skill 意圖判斷）把剛剛那份壓縮後的內容載進來

新對話一開始的 context 就是壓縮版，不是重讀原始 transcript——這才是真正省下來的地方。**代價是你要願意切到新對話，而不是無縫接回原本那條串。**
